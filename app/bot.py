...
1. DISCORD_BOT_TOKEN environment variable (RECOMMENDED)
2. BOT_TOKEN environment variable (legacy support)
3. token.txt file at repository root (NOT RECOMMENDED - local dev only)
...
def read_bot_token() -> Optional[str]:
	...
	Priority order:
	1. DISCORD_BOT_TOKEN environment variable (RECOMMENDED)
	2. BOT_TOKEN environment variable (legacy compatibility)
	3. token.txt file in repo root (NOT RECOMMENDED - local dev only)

	Returns:
		str: Discord bot token, or None if not found
	"""
	# Priority 1: DISCORD_BOT_TOKEN (recommended)
	token = os.getenv("DISCORD_BOT_TOKEN")
	if token:
		logger.info("✅ Token loaded from DISCORD_BOT_TOKEN environment variable")
		return token.strip()
	...
	# Priority 3: token.txt file (NOT RECOMMENDED)
	root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
	token_file = os.path.join(root, "token.txt")

	if os.path.exists(token_file):
		logger.warning("⚠️ WARNING: Loading token from token.txt file")
		logger.warning("⚠️ This method is INSECURE and should only be used for local development")
		logger.warning("⚠️ Please migrate to environment variables (.env file)")
		logger.warning(f"⚠️ Token file location: {token_file}")
		try:
			with open(token_file, "r", encoding="utf-8") as f:
				line = f.readline().strip()
				if line:
					logger.info("✅ Token loaded from token.txt (insecure)")
					return line
				else:
	...
	logger.error("❌ No token found in environment variables or token.txt")
	logger.error("💡 To fix this:")
	logger.error("   1. Create a .env file with: DISCORD_BOT_TOKEN=your-token-here")
	logger.error("   2. Or set environment variable: set DISCORD_BOT_TOKEN=your-token-here")
	logger.error("   3. See docs/HUONG_DAN_BAO_MAT_TOKEN.md for detailed instructions")
	return None
